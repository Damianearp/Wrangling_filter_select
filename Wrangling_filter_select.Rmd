...
From what we're observing, we have different titles, genre's, length ratings and even reviews on the movies. 
...
Above we can safely conclude that our mean is 5.93!

```{r}
median(movies$rating)
```

Our Median budget is going to be 6.1!
...
```{r}
newdata2<-na.omit(movies$length)
```

(newdata2 represents the movie length over 60 minutes)
...
- What is the average movie rating? The movie rating average is 5.93

- What is the median budget for a movie? our Median budget is 3,000,000
